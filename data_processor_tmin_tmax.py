...
class DataProcessor:
    ...
    def update_avg(self, df):
        ...
        '''
        data = df.copy()
        self.l.log_info('Converting dloc column to timestamp')
        data['time'] = data.dloc.apply(lambda x: pd.to_datetime(x, format=self.dt_format))
        data = data.set_index('time')
        self.l.log_info('Getting the average temperature and precipitation of last 2 days')
        avg = data[['ides','idmun','tmax','tmin']].groupby(['ides', 'idmun']).rolling('2D').mean()
         
 
        file = 'historico\\average_2_days.csv'
        if file.split('\\')[-1] not in os.listdir('historico'):
